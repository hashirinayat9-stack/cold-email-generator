...
# --- API Key Setup ---
# Uses st.secrets to securely manage keys locally (.streamlit/secrets.toml) and on Streamlit Cloud
...
# --- RAG Setup (FIXED for Relative Paths) ---
embeddings = HuggingFaceEmbeddings(
    model_name="BAAI/bge-m3"
)

# Define the local, relative path that matches your project structure:
# The path assumes the DB files are in ./resume_portfolio-db/resume_portfolio
LOCAL_DB_PATH = os.path.join(".", "resume_portfolio-db", "resume_portfolio")

# Check if the database path exists before trying to load it
if not os.path.isdir(LOCAL_DB_PATH):
    st.error(f"Error: The Chroma DB path was not found: {LOCAL_DB_PATH}.")
    st.markdown("Please ensure the database files have been cloned/copied into the correct folder.")
    st.stop()

# Initialize Chroma and Retriever using the local path
